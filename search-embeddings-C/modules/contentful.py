...
def get_all_entries(
    content_type: str, page_size: int = PAGE_SIZE, include: int = 2
) -> Generator:
        ...
        while True:
            try:
                ...
                if "Response size too big" in str(e) and page_size > 1:
                    ...
                    log.warning(
                        "Response too large. Reducing page size to %d and retrying...",
                        page_size,
                    )
                else:
                    raise
    ...
    for include in [2, 1, 0]:
        for attempt in range(MAX_RETRIES):
            try:
                ...
                wait = float(getattr(e, "reset_time", 2**attempt))
                log.warning(
                    "Rate limited (entry=%s). Retrying in %.0fs...", entry_id, wait
                )
                time.sleep(wait)
        else:
            raise RuntimeError(
                f"Failed to fetch entry {entry_id} after {MAX_RETRIES} retries"
            )

    raise RuntimeError(
        f"Failed to fetch entry {entry_id} — response too large even with include=0"
    )
