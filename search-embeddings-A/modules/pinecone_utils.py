...
SPARSE_BATCH_SIZE = 96  # pinecone-sparse-english-v0 limit


def get_sparse_vectors(texts: list) -> list:
    results = []
    for i in range(0, len(texts), SPARSE_BATCH_SIZE):
        batch = texts[i: i + SPARSE_BATCH_SIZE]
        results.extend(pc.inference.embed(
...
def delete_by_type(content_type: str):
    ...
    index.delete(
        filter={"type": {"$eq": content_type}},
        namespace=PINECONE_NAMESPACE,
    )
    log.info("Deleted all vectors with type='%s' from namespace '%s'", content_type, PINECONE_NAMESPACE)
